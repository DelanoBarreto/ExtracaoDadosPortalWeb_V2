...
export async function GET(request: Request) {
  try {
    ...
    const base = () => {
      let q = supabaseAdmin.from('tab_noticias').select('id', { count: 'exact', head: true });
    ...
    const [total, publicado, rascunho, arquivado] = await Promise.all([
      ...
      // Null, rascunho ou draft (or() correto do PostgREST)
    ...
    console.error('❌ Erro no noticias-counts:', error);
